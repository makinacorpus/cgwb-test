...
from test.jpcw.setuphandlers import setup_catalog
from test.jpcw import upgrades
...
class FileTestJpcwMigrator(BlobMigrator):
    src_portal_type = 'FileTestJpcw'
    src_meta_type = 'FileTestJpcw'
    dst_portal_type = 'FileTestJpcw'
    dst_meta_type = 'FileTestJpcw'


class TestJpcw_migrate(grok.View):
    ...
    def custom_hook_blob(self):
        self.migrate_fileTestJpcw_to_blob()
    ...
    def custom_hook_after_plone_upgrade(self):
        ...
        ps.runImportStepFromProfile(
            'profile-TestJpcw.skin:default', "skins", run_dependencies=False)
    ...
    @onlyonce
    def migrate_fileTestJpcw_to_blob(self):
        upgrades.log('Migrating FileTestJpcw to blob')
        walker = CustomQueryWalker(self.context, FileTestJpcwMigrator, full_transaction=True)
        walker.go()
        upgrades.commit(self.context)
    ...
    def log_files(self, idx='1'):
        ptypes = ['Image', 'FileTestJpcw', 'File',
