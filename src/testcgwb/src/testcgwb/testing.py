...
class TestcgwbLayer(PloneSandboxLayer):
    ...
    """Layer to setup the testcgwb site"""
    class Session(dict):
        def set(self, key, value):
            self[key] = value

    def setUpZope(self, app, configurationContext):
        """Set up the additional products required for the ) site testcgwb.
        ...
        # -----------------------------------------------------------------------
        import testcgwb
        self.loadZCML('configure.zcml', package=testcgwb)
    ...
    def setUpPloneSite(self, portal):
        ...
        self.applyProfile(portal, 'testcgwb:default')


TESTCGWB_FIXTURE = TestcgwbLayer(name='Testcgwb:Fixture')
...
TESTCGWB_SIMPLE = SimpleLayer(name='Testcgwb:Simple')
TESTCGWB_INTEGRATION_TESTING = IntegrationTesting(name="Testcgwb:Integration")
TESTCGWB_FUNCTIONAL_TESTING = FunctionalTesting(name="Testcgwb:Functional")
TESTCGWB_SELENIUM_TESTING = FunctionalTesting(
    bases=(SELENIUM_TESTING, TESTCGWB_FUNCTIONAL_TESTING,),
...
class Browser(z2.Browser):  # pragma: no cover
    ...
    def new(cls, url=None, user=None, passwd=None, headers=None, login=False):
        ...
        app = TESTCGWB_FUNCTIONAL_TESTING['app']
        portal = TESTCGWB_FUNCTIONAL_TESTING['portal']
        if not url:
